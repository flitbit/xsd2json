		...
		if (typeof xsdText != "string")
		{
			...
			convertXsdTextToJavascript(modifiedText, function(err, jsXsd){
				...
				else
				{
					...
					var xsdRoot = getXsdRoot(jsXsd);
					var xsdOrigin = getXsdOrigin(xsdRoot);
					schema.xsdOrigin = xsdOrigin;
					...
					executeConverter(schema, xsdRoot, xsdOrigin, function(err, data){
						if (err){
							callback(err);
						}
						else{
...
function executeConverter(schema, jsobj, xsdOrigin, callback){
	var keys = Object.keys(jsobj);
	keys.forEach(function(propName){
		executeConverterRecursive(schema, jsobj, propName, xsdOrigin, callback);
	});
}

function executeConverterRecursive(schema, jsobj, propName, xsdOrigin, callback){
	try{
		...
		else
		{
            ...
            // get the property name without the namespace.
			...
			try{
				...
				console.log("Node converter for ".concat(propName).concat(" found."));
			}
			catch(e){
				// simply means that the node converter was not found.
				console.log("Node converter for ".concat(propName).concat(" not found."));
			}
			if (nodeConverter){
				jsobj[propName] = nodeConverter.getJsonNode(schema, jsobj, targetObj);
			}
            else{
                if (targetObj != null && typeof targetObj === 'object'){
                    var keys = Object.keys(targetObj);
                    keys.forEach(function(key){
                        executeConverterRecursive(schema, targetObj, key);
                    });
                }
            }
			if (callback){
